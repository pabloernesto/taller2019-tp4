...
void CarView::render() {
  ...
  if (!car.isDead()){
    this->camara.renderMe(position, this->car.GetSize(), this->imageAlive, M_PI + this->car.GetAngle());
  } else {
    this->camara.renderMe(position, this->car.GetSize(), this->imageDead, M_PI + this->car.GetAngle());
  }
}
